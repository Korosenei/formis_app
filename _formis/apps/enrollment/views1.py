...
def candidature_approve(request, pk):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            logger.info(f"✅ Candidature approuvée: {candidature.numero_candidature}")

            # Envoyer email de notification d'approbation
            try:
                EmailCandidatureManager.send_candidature_evaluated(candidature)
                logger.info(f"✅ Email d'approbation envoyé à {candidature.email}")
            except Exception as e:
                logger.error(f"❌ Erreur envoi email d'approbation: {str(e)}")

            # IMPORTANT: Créer le compte utilisateur APPRENANT
            try:
                user = create_user_from_candidature(candidature)

                if user:
                    logger.info(
                        f"✅ Compte créé automatiquement pour {candidature.email}\n"
                        f"   Matricule: {user.matricule}\n"
                        f"   L'apprenant devra s'inscrire lors de sa première connexion"
                    )
                else:
                    logger.warning(
                        f"⚠️ Compte non créé (existe déjà?) pour {candidature.email}"
                    )
            except Exception as e:
                logger.error(f"❌ Erreur création compte: {str(e)}")
                # Ne pas bloquer l'approbation si la création du compte échoue
                # L'admin pourra créer le compte manuellement

        return JsonResponse({
            'success': True,
            'message': 'Candidature approuvée avec succès. Le compte utilisateur a été créé.',
            'decision': 'APPROUVEE',
            'date_decision': candidature.date_decision.strftime('%d/%m/%Y à %H:%M')
        })

    except Exception as e:
        logger.error(f"❌ Erreur approbation candidature: {str(e)}", exc_info=True)
        return JsonResponse({
            'success': False,
            'message': f'Une erreur est survenue: {str(e)}'
        }, status=500)
...
class InscriptionAvecPaiementView(TemplateView):
    """Vue publique pour s'inscrire avec le token"""
        ...
        try:
            ...
            with transaction.atomic():
                ...
                inscription = Inscription.objects.create(
                    candidature=candidature,
                    apprenant=None,  # Sera créé après paiement
                ...
                # Déterminer le montant de la première tranche
                if type_paiement == 'UNIQUE':
                    montant_a_payer = montant_du
                    tranche = None
                    description = f"Paiement unique - Inscription {candidature.filiere.nom}"
                else:
                    tranche = plan.tranches.filter(est_premiere_tranche=True).first()
                    if not tranche:
                        tranche = plan.tranches.order_by('numero').first()
                ...
                logger.info(f"Inscription et paiement créés pour candidature {candidature.numero_candidature}")
